...
if [[ "${GIT_PROMPT_MODE}" == "OLD" ]] ; then
    ...
    if [[ -n $(command -v fzf) ]] ; then
        ...
        plugins=(git virtualenv fzf zshmarks zsh-autosuggestions docker ssh zsh-syntax-highlighting zoxide git-prompt.zsh)
    else
...
if [[ "${GIT_PROMPT_MODE}" == "OLD" ]] ; then
    ...
    export PROMPT='%(?,,%{$fg[red]%}FAIL%{$reset_color%}${NEWLINE})${NEWLINE}%{%F{197}%}${DOCKERPROMPT}$(virtualenv_prompt_info)%{$reset_color%}%{%F{'"${PROMPT_COLOR}"'}%}${PROMPT_STRING}%{$reset_color%}%{$reset_color%} %{$DIRECTORY_COLOR%}%1d%{$reset_color%} $(gitprompt)${NEWLINE}%F{241}%% %F{reset_color}'

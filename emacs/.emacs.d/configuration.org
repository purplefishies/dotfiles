...
#+begin_example
<style>
/* Shared styling for all src blocks */
pre.src {
  border: 2px solid #444;
  border-radius: 6px;
  padding: 12px;
  ...
  /* visually pleasing monospace formatting */
  font-family: monospace;
  font-size: 0.95em;

  /* optional subtle background to make blocks stand out */
  background-color: #1e1e1e;
  color: #e5e5e5;
  overflow-x: auto;        /* allow scroll if lines are long */
}

/* Optional: label the language (example: Bash) */
pre.src-bash:before {
  content: "shell> ";
}
</style>

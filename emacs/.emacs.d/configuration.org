...
#+RESULTS:
| jmd/git-auto-commit-python | my-auto-commit |
...
(defun insert-ansi-org (txtfile)
  "Convert ANSI TXTFILE to HTML and PDF and insert conditional Org blocks
for both HTML and LaTeX export."
  ...
  (let* ((orgfile (buffer-file-name))
    ...
    (unless (executable-find "ansi2html")
      (error "ansi2html not found (pip install ansi2html)"))
    ...
    (let ((exit-code
      ...
      (unless (eq exit-code 0)
        (error "ansi2html failed; see *ansi2html-log*")))

    ;; -----------------------------
    ;; Extract only <pre>…</pre>
    ;; -----------------------------
    (let* ((full-html (with-current-buffer htmlbuf (buffer-string)))
           (pre-start (string-match "<pre[^>]*>" full-html))
           (pre-tag   (match-string 0 full-html))  ;; must capture before second match
           (pre-end   (string-match "</pre>" full-html))
           (inner (substring full-html
      ...
      (let* ((raw (with-temp-buffer
        ...
        ;; Write nice compact HTML
        (with-current-buffer htmlbuf
          (erase-buffer)
          (insert
           (format "<div class=\"ansi-output\" style=\"width:%s; display:inline-block; white-space:pre; padding:0.2em; background:#111; color:#eee; border-radius:4px;\">\n%s\n</div>\n"
                   css-width inner))))))
    ...
    ;; -----------------------------
    (with-current-buffer logbuf
      (insert (format "\nRunning wkhtmltopdf → %s\n" pdf-file)))
    (let ((exit-code
           (call-process "wkhtmltopdf" nil logbuf nil html-file pdf-file)))
      (unless (eq exit-code 0)
        (error "wkhtmltopdf failed")))

    ;; -----------------------------
    ;; Insert Org markup for both exports
    ;; -----------------------------
    (insert
     (format
"# ------------------------------------------------------------\n\
...
#+HTML: </div>\n\n\
# ------------------------------------------------------------\n\
# LaTeX export block (PDF)\n\
#+LATEX: \\begin{center}\n\
#+LATEX: \\includegraphics[width=0.9\\linewidth]{%s}\n\
#+LATEX: \\end{center}\n\n"
             (file-relative-name html-file orgdir)
             (file-relative-name pdf-file orgdir)))
    ...
    (message "Inserted ANSI block for HTML + LaTeX: %s" unique-base)))

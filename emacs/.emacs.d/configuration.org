...
(defun insert-ansi-org (txtfile)
  "Convert ANSI TXTFILE to HTML and insert an Org INCLUDE block for it.
Uses python ansi2html, strips full HTML headers, and keeps ONLY <pre> content.
Produces compact HTML for perfect wkhtmltopdf output."
  (interactive "fANSI text file: ")

  ;; Must be in an org buffer
  ...
  (let* ((orgfile (buffer-file-name))
         ...
         (unique (format "%s_%s.html"
                         (file-name-base src)
                         (format-time-string "%Y%m%d-%H%M%S")))
         (dest (expand-file-name unique target-dir))
         (logbuf  (get-buffer-create "*ansi2html-log*"))
         (htmlbuf (get-buffer-create "*ansi2html-output*")))
    ...
    (unless (executable-find "ansi2html")
      (error "ansi2html not found. Install: pip install ansi2html"))

    ;; Clear buffers
    (with-current-buffer logbuf (erase-buffer))
    (with-current-buffer htmlbuf (erase-buffer))

    ;; Log start
    (with-current-buffer logbuf
      (insert (format "Running ansi2html on %s\nOutput → %s\n\n" src dest)))

    ;; -----------------------------
    ;; Run ansi2html → FULL HTML into htmlbuf
    ;; -----------------------------
    (let ((exit-code
           (with-current-buffer htmlbuf
             (call-process "ansi2html" src t nil))))
      (unless (eq exit-code 0)
        (with-current-buffer logbuf
          (insert (format "ERROR: exit %s\n" exit-code)))
        (display-buffer logbuf)
        (error "ansi2html failed")))

    ;; -----------------------------
    ;; STRIP everything except the <pre> ... </pre>
    ;; -----------------------------
    (let* ((full-html (with-current-buffer htmlbuf (buffer-string)))
       ;; First match <pre…> and capture the tag BEFORE running ANY other match
       (pre-start (string-match "<pre[^>]*>" full-html))
       (pre-tag   (match-string 0 full-html))   ; save now, before match-data gets overwritten
       ;; Now this will NOT overwrite pre-tag
       (pre-end   (string-match "</pre>" full-html))
       (inner (if (and pre-start pre-end pre-tag)
                  ;; correct: skip exactly the number of characters in the <pre> tag
                  (substring full-html
      ...
      ;; -----------------------------
      ;; Compute text width from RAW ANSI file
      ;; -----------------------------
      (let* ((raw (with-temp-buffer
        ...
        ;; -----------------------------
        ;; Create FINAL compact HTML
        ;; -----------------------------
        (with-current-buffer htmlbuf
          (erase-buffer)
          (insert
           (format
"<div class=\"ansi-output\" style=\"width:%s; display:inline-block; white-space:pre; padding:0.2em; background:#111; color:#eee; border-radius:4px;\">\n%s\n</div>\n"
                   css-width inner)))))


    ;; -----------------------------
    ;; Write compact HTML to dest
    ;; -----------------------------
    (with-current-buffer htmlbuf
    ...
    ;; Insert Org block
    (insert
     (format "#+HTML: <div class=\"ansi-output\">\n#+INCLUDE: \"%s\" export html\n#+HTML: </div>\n\n"
             (file-relative-name dest orgdir)))

    ;; Log success
    (with-current-buffer logbuf
      (goto-char (point-max))
      (insert (format "SUCCESS: wrote %s\n" dest)))
    (display-buffer logbuf)

    (message "Inserted ANSI block: %s" dest)))
#+end_src

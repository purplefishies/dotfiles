...
(defun my-org-screenshot ()
  "Take a screenshot into YEAR/Month subdirectory based on the Org file name,
insert a link, and git add the image."
  (interactive)
  ...
  (let* ((base-name (file-name-base (buffer-file-name)))  ;; e.g., 'work'
         ...
         (month     (format-time-string "%b"))            ;; "Nov"
         (root-dir  (file-name-directory (buffer-file-name)))
         (dir       (expand-file-name (format "%s/%s/%s" base-name year month) root-dir))
         (filename  (format "%s_%s.png"
                            (format-time-string "%Y%m%d_%H%M%S")
                            (file-name-nondirectory (make-temp-name ""))))
         (file-rel  (format "%s/%s/%s/%s" base-name year month filename))
         (file-abs  (expand-file-name file-rel root-dir)))

    ;; ensure directory exists
    (make-directory dir t)

    ;; screenshot (ImageMagick `import` tool)
    (call-process "import" nil nil nil file-abs)

    ;; insert link
    (insert (concat "[[./" file-rel "]]"))
    (org-display-inline-images)

    ;; git add the image (repo already exists)
    (message "git add %s" file-rel)
    (call-process "git" nil nil nil "add" file-abs)

    (message "Screenshot saved + linked + staged: %s" file-rel)))

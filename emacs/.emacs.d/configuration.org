...
(setq org-html-head
"<style>
/* Base styling for all src blocks */
pre.src {
...
/* Language label for bash blocks */
pre.src-bash:before {
  content: \"shell%\";
  position: absolute;
  top: -0.8rem;
...
<script>
// Add a persistent \"Copy code\" button to each code block
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('pre.src').forEach(function(pre) {
    ...
    btn.addEventListener('click', () => {
      ...
      // ✅ Clone the node so we can strip UI elements before copying
      const clone = pre.cloneNode(true);

      // Remove the button inside the clone

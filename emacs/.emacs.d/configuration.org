...
(defun my-org-insert-image ()
  "Prompt for an image, copy it to <basename>/<YEAR>/<Mon>/, insert link, show inline, and git add it."
  (interactive)
  (unless (buffer-file-name)
    (error "Current buffer is not visiting a file"))

  (let* ((src (read-file-name "Select image file: "))
         (base (file-name-base (buffer-file-name)))   ;; inbox from inbox.org
         (year  (format-time-string "%Y"))
         (month (format-time-string "%b"))            ;; Jan/Feb/Mar...
         (root  (file-name-directory (buffer-file-name)))

         ;; create target directory structure inbox/2026/Feb/
         (dest-dir (expand-file-name (format "%s/%s/%s" base year month) root))

         ;; keep original filename
         (dest-file (expand-file-name (file-name-nondirectory src) dest-dir))

         ;; link relative to the .org file location
         (rel-path (file-relative-name dest-file root)))

    ;; ensure dir exists
    (make-directory dest-dir t)

    ;; copy file into the org-document's tree
    (copy-file src dest-file t)

    ;; insert org link
    (insert (format "[[./%s]]" rel-path))
    (org-display-inline-images)

    ;; git add: use magit when available, fallback to git CLI
    (message "Staging file: %s" rel-path)
    (if (and (fboundp 'magit-call-git)
             ;; ensure we’re really inside a git repo
             (= (call-process "git" nil nil nil "rev-parse" "--is-inside-work-tree") 0))
        (magit-call-git "add" dest-file)
      (call-process "git" nil nil nil "add" dest-file))

    (message "Image copied, linked, and staged: %s" rel-path)))
...
;;                             "#+title: %<%Y-%m-%d>\n"))))
#+end_src
** Org Roam ignore regular links
#+begin_src emacs-lisp
(setq org-roam-graph-exclude-matcher '("https?://"))
(setq org-roam-graph-ignore-missing-nodes t)
#+end_src
...
(defvar my-auto-commit-enabled t
  "If non-nil, auto-commit on save is enabled.")

(defun my-toggle-auto-commit ()
  "Toggle automatic git commits on save."
  (interactive)
  (setq my-auto-commit-enabled (not my-auto-commit-enabled))
  (message "Auto-commit is now %s"
           (if my-auto-commit-enabled "ENABLED" "DISABLED")))


(defun my-auto-commit ()
  "Auto commit Org or shell-script files to git on save."
  (when (and my-auto-commit-enabled
             buffer-file-name
             (or (string-match "\\.org\\'" buffer-file-name)
                 (derived-mode-p 'sh-mode)))
    (let* ((default-directory (locate-dominating-file buffer-file-name ".git"))
           (msg (format ":robot: Auto-commit: %s"
...
;; (setq org-html-htmlize-output-type 'css)          ;; no inline styles
;; (setq org-html-head-include-default-style nil)    ;; no default Org CSS
;; (setq org-html-head-include-scripts nil)          ;; optional (cleaner HTML)
...
/* ---- BEST PRACTICE ----
   Layout belongs to:  div.org-src-container
   Visual styling belongs to: pre.src
   ------------------------------------------------------------ */

/* Controls alignment and layout of code blocks */
div.org-src-container {
  margin: 1.2em auto;      /* center within content flow */
}

/* Controls appearance inside the block */
pre.src {
  background: #ffffff;
  ...
  box-sizing: border-box;  /* padding + border do not widen it */
...
/* Show language marker: Only Bash */
pre.src-bash:before {
  content: \"shell%\";
  font-size: 0.70rem;
  font-weight: bold;
...
/* Copy button (ChatGPT-style, always visible) */
.copy-code-btn {
  position: absolute;
  top: 6px;
  right: 6px;
...
.copy-code-btn:hover { background: #e5e5e5; }

</style>

<script>
// Insert ChatGPT-style \"Copy code\" buttons
...
(setq org-html-image-inline-data t)
(setq org-latex-image-inline-data nil)
...
(defvar jmd/git-auto-commit-python-enabled t
  "If non-nil, auto-commit Python files on save is enabled.")

(defun jmd/toggle-git-auto-commit-python ()
  "Toggle automatic git commits for Python files on save."
  (interactive)
  (setq jmd/git-auto-commit-python-enabled
        (not jmd/git-auto-commit-python-enabled))
  (message "Python auto-commit is now %s"
           (if jmd/git-auto-commit-python-enabled "ENABLED" "DISABLED")))

(defun jmd/git-auto-commit-python ()
  "Automatically commit Python files to git on save."
  (when (and jmd/git-auto-commit-python-enabled
             buffer-file-name
...
(remove-hook 'after-save-hook #'jmd/git-auto-commit-python)
(add-hook 'after-save-hook #'jmd/git-auto-commit-python)
#+end_src


* Ansi Insert
#+begin_src emacs-lisp
(defun insert-ansi-org (txtfile)
  "Convert ANSI TXTFILE to HTML and PDF and insert Org blocks for HTML + LaTeX export."
  (interactive "fANSI text file: ")

  (unless (derived-mode-p 'org-mode)
    (error "Not in an Org buffer"))

  (let* ((orgfile   (buffer-file-name))
         (orgdir    (file-name-directory orgfile))
         (basename  (file-name-base orgfile))
         (year      (format-time-string "%Y"))
         (month     (format-time-string "%b"))
         (target-dir (expand-file-name (format "%s/%s/%s/" basename year month)
                                       orgdir))
         (src       (expand-file-name txtfile))
         (unique-base (format "%s_%s"
                              (file-name-base src)
                              (format-time-string "%Y%m%d-%H%M%S")))
         (html-file (expand-file-name (concat unique-base ".html") target-dir))
         (pdf-file  (expand-file-name (concat unique-base ".pdf")  target-dir))
         (logbuf    (get-buffer-create "*ansi2html-log*")))

    (make-directory target-dir t)

    (unless (executable-find "ansi2html")
      (error "ansi2html not found in PATH"))
    (unless (executable-find "wkhtmltopdf")
      (error "wkhtmltopdf not found in PATH"))

    (with-current-buffer logbuf
      (erase-buffer)
      (insert (format "ansi2html → %s\nwkhtmltopdf → %s\n\n"
                      html-file pdf-file)))

    ;; --- Run ansi2html and capture full HTML as string ---
    (let* ((full-html
            (with-temp-buffer
              (let ((exit (call-process "ansi2html" src t nil)))
                (unless (eq exit 0)
                  (with-current-buffer logbuf
                    (insert (format "ansi2html exit %s\n" exit)))
                  (error "ansi2html failed"))
                (buffer-string))))
           ;; --- Extract <pre>…</pre> using your working pre-tag trick ---
           (pre-start (string-match "<pre[^>]*>" full-html))
           (pre-tag   (and pre-start (match-string 0 full-html)))
           (pre-end   (and pre-start (string-match "</pre>" full-html)))
           (inner
            (if (and pre-start pre-end pre-tag)
                (substring full-html
                           (+ pre-start (length pre-tag))
                           pre-end)
              (error "Could not find <pre> block in ansi2html output"))))

      ;; --- Compute width from original ANSI file ---
      (let* ((raw (with-temp-buffer
                    (insert-file-contents src)
                    (buffer-string)))
             (lines     (split-string raw "\n"))
             (max-width (max 20 (apply #'max (mapcar #'length lines))))
             (css-width (format "%dch" max-width))
             (final-html
              (format
               "<div class=\"ansi-output\" style=\"width:%s; display:inline-block; white-space:pre; padding:0.3em; background:#111; color:#eee; border-radius:4px;\">\n%s\n</div>\n"
               css-width inner)))

        ;; --- Write compact HTML snippet ---
        (with-temp-buffer
          (insert final-html)
          (write-region (point-min) (point-max) html-file nil 'silent))))

    ;; --- Run wkhtmltopdf to create PDF ---
    (with-current-buffer logbuf
      (insert (format "Running: wkhtmltopdf %s %s\n" html-file pdf-file)))
    (let ((exit (call-process "wkhtmltopdf" nil logbuf nil html-file pdf-file)))
      (unless (eq exit 0)
        (with-current-buffer logbuf
          (insert (format "wkhtmltopdf exit %s\n" exit)))
        (error "wkhtmltopdf failed")))

    ;; --- Insert Org blocks (HTML-only + LaTeX-only) ---
    (let ((rel-html (file-relative-name html-file orgdir))
          (rel-pdf  (file-relative-name pdf-file  orgdir)))
      (insert
       (format
"# HTML export
#+HTML: <div class=\"ansi-output\">
#+INCLUDE: \"%s\" export html
#+HTML: </div>

# LaTeX export (PDF)
#+LATEX: \\begin{center}
#+LATEX: \\includegraphics[width=0.95\\linewidth]{%s}
#+LATEX: \\end{center}
...
"
    ...
    (with-current-buffer logbuf
      (goto-char (point-max))
      (insert "SUCCESS\n"))
    (display-buffer logbuf)
    (message "insert-ansi-org: HTML+PDF → %s / %s" html-file pdf-file)))
...
(defun jmd/org-download-dir ()
  "Return <stem>/<YEAR>/<Mon>/ relative to the Org file."
  (let* ((buf (or (buffer-file-name)
         ...
         (target (expand-file-name
                  (format "%s/%s/%s/" stem year mon) dir)))
    (unless (file-directory-p target)
      (make-directory target t))
    target))

(defun jmd/org-download--target-file (suffix)
  "Generate a complete target filename for image saving."
  (let* ((dir  (jmd/org-download-dir))
         (name (format "%s%s"
                       (format-time-string "%Y-%m-%d_%H-%M-%S")
                       suffix)))
    (expand-file-name name dir)))

(defun jmd/org-download-clipboard ()
  "Paste clipboard image into year/month directory beside Org file."
  (interactive)
  (let* ((file (jmd/org-download--target-file ".png"))
         (org-download-clipboard-file file)
         (org-download-link-format "[[file:%s]]\n"))
    (org-download-clipboard)))

(defun jmd/org-download-screenshot ()
  "Screenshot into year/month directory beside Org file."
  (interactive)
  (let* ((file (jmd/org-download--target-file "_screenshot.png"))
         (org-download-screenshot-file file)
         (org-download-link-format "[[file:%s]]\n"))
    (org-download-screenshot)))

;; Disable org-download default Pictures/ directory completely
(setq org-download-image-dir nil)

;; Keybindings
(with-eval-after-load 'org
  (define-key org-mode-map (kbd "C-c i c") #'jmd/org-download-clipboard)
  (define-key org-mode-map (kbd "C-c i s") #'jmd/org-download-screenshot))

#+end_src

* Customized HTML output
#+begin_src text
Here’s the minimal, clean way to add a “copy” button like that to your Org → HTML publishing output in Emacs.
...
<nav class="md-code__nav">
  <button
    ...
    data-clipboard-target="#__code_2 > code"
    data-md-type="copy"
    title="Copy to clipboard">
  </button>
</nav>

✅ How to inject this into exported Org-mode HTML

Org already gives you org-html-head and org-html-head-extra hooks to inject HTML + JS.

You want:

A copy button placed next to every <pre><code> block

A small JS snippet that wires it up to Clipboard API (or clipboard.js)

Below is the cleanest working solution.

✅ Add this to your publishing config:
1. Add the JS + CSS buttons to <head>
This:

✔ Wraps every <pre> block in a container
✔ Inserts a nav.md-code__nav > button.md-code__button
✔ Makes the button copy the code
✔ Gives a visual flash after copying

Works perfectly with org-publish-project-alist.

🟦 If you prefer, you can make it even closer to your sample markup

Your sample uses:

<button data-clipboard-target="#__code_2 > code" data-md-type="copy">

To match that exactly, change the JS button generation to:

btn.setAttribute("data-clipboard-target", "#codeblock_" + i + "> code");
btn.setAttribute("data-md-type", "copy");

and assign IDs to code blocks:

block.id = "codeblock_" + i;

Just tell me if you want the full version.

✅ Result

Your published Org HTML will now have:

+-------------------------------+
| [copy button]   <code block> |
+-------------------------------+

Exactly like MkDocs / Material-style copy buttons.

Want it to work only for source blocks (not example blocks)?
Want it themed (dark/light)?
Want clipboard.js instead of browser API?

Just tell me — I'll tailor it to your publication style.
#+end_src

#+begin_src emacs-lisp
;; (setq org-html-head-extra
;; "<style>
;; .md-code__nav { position: absolute; right: 6px; top: 6px; }
...
;;   /* Tabler 'copy' icon as inline SVG data URI */
;;   background-image: url('data:image/svg+xml;utf8,
;; <svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 24 24\" stroke-width=\"1.5\" stroke=\"%23000\" fill=\"none\" stroke-linecap=\"round\" stroke-linejoin=\"round\">
